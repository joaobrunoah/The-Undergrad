...
export default function MessagesList(props) {
  ...
  this.delete = async item => {
    try {
      let other_user_name = await System.getUserInfo(item.key)
        .then(user => {
      ...
      Alert.alert(
        ...
        props.language === 'br'
          ...
          : props.language === 'usa'
          ? `Delete the messages of ${other_user_name}?`
          : `Apagar as mensagens de ${other_user_name}?`,
        [
          {
            text:
          ...
          },
          {
            text: 'OK',
            onPress: async () =>
              await System.deleteMessages(props.uid, item.key),
          },
        ],
        {cancelable: false},
      );
    ...
    <View
      ...
      {props.loading ? (
        ...
        <SwipeListView
          ...
          renderItem={({item}) => {
            return (
              <Message data={item} msg={this.lastMsg(item)} uid={props.uid} />
            );
          }}
          renderHiddenItem={({item}) => (
            <View
              ...
              <TouchableOpacity
                ...
                <FontAwesomeIcon
                  name={'trash-o'}
                  style={{color: 'white', fontWeight: 'bold', paddingRight: 7}}
                  size={35}
                />
              </TouchableOpacity>
            </View>
